setwd('C:\\Users\\cdron\\Desktop\\Cleanup\\Misc\\kaggle\\Caterpillar_Tube_Pricing\\competition_data')
getwd()
options(scipen = 10)
...
# ### Train randomForest on dtrain_cv and evaluate predictions on dtest_cv
 set.seed(123)
# rf1 = randomForest(dtrain_cv$cost~., dtrain_cv[,-match(c("id", "cost"), names(dtrain_cv))], ntree = 20, do.trace = 2)
# 
...
rf = randomForest(log(train$cost + 1)~., train[,-match(c("id", "cost"), names(train))], ntree = 25, do.trace = 2)
...
write.csv(submitDb, "../submit_july3rd_5.csv", row.names = FALSE, quote = FALSE)
