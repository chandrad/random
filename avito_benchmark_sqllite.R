...
trainSearchStreamContextual <- fetch(db, "select HistCTR,Position,IsClick from trainSearchStream where ObjectType=3", 10 * million)
m <- nrow(trainSearchStreamContextual)

# Create stratified sample 
sampleSize <- 10* million #100 * million
...
trainSampleIndex <- createDataPartition(y = trainSearchStreamContextualSample$IsClick, p = .88, list = FALSE)
...
testSearchStreamContextual <- fetch(db, "select TestId, HistCTR, Position from testSearchStream where ObjectType=3")
...
write.csv(submissionData, 'submission_july04_3.csv', sep=",", dec=".", col.names=TRUE, row.names=FALSE)
